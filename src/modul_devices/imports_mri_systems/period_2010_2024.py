#============================================================================================
# V. IMPORT OF MAGNETIC RESONANCE IMAGING (MRI) SCANNERS
#============================================================================================
...
# -------------------------------------------------------------------
    ...
    file_path = "./data/raw"
    files = [f for f in os.listdir(file_path) if f.startswith("901813_MRI_Systems_") and f.endswith(".xlsx")] 
...
# Load and merge all files
    ...
    for file in files:
    # extract the year from the filename; for example, "902213_MRI_Systems_2024.xlsx" becomes "2024"
...
# Combine all DataFrames into a single one
    ...
    df_mri_scanners = pd.concat(list_df, ignore_index=True)
...
# Remove columns that are completely empty (containing only NaN)
    df_mri_scanners = df_mri_scanners.dropna(axis=1, how='all')
...
# Remove unnecessary columns
    ...
    df_mri_scanners = df_mri_scanners.drop(columns=columns_to_remove, errors="ignore")


# Rename useful columns   
    df_mri_scanners = df_mri_scanners.rename(columns={
...
# Add a standardized product name
    df_mri_scanners["Product_Name"] = "MRI Scanners"

# Format the "Trade Value 1000USD" column
    df_mri_scanners["Trade Value 1000USD"] = (
    df_mri_scanners["Trade Value 1000USD"]
...
# Remove rows with missing values in the "Trade Value 1000USD" column
    df_mri_scanners = df_mri_scanners.dropna(subset=["Trade Value 1000USD"])
...
# Create the "Country_Code" column
    df_mri_scanners["Country_Code"] = df_mri_scanners["Country"].apply(get_country_code)

# Remove rows with missing values in the "Country_Code" column
    df_mri_scanners = df_mri_scanners.dropna(subset=["Country_Code"])

# Add placeholder columns for variables that will be calculated in later modules
    df_mri_scanners["Units_per_Million_Inhabitants"] = np.nan
    ...
    df_mri_scanners = df_mri_scanners[final_columns]

    #print(df_mri_scanners.head())
